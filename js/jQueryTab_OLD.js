...
var jQueryTab = {
	init:function(opts){
			...
			$(self.opts.tabclass) .on((self.opts.opentabonhover)?'click.jqeryTab, hover.jqeryTab' :'click.jqeryTab' , "a", function(){										// when tab is clicked
	...
	addHeader: function(){
			...
			$(self.opts.tabclass).find("a").each(function(){													// iterate for each  tabs
				$tab_content = $(self.opts.containerclass).eq($(this).parent().index());						// get corresponding tab content
	...
	setCookie: function(value){
			...
			document.cookie = [self.opts.cookie.name, '=', encodeURIComponent(value), expires, path, domain, secure].join('');
	},
...
$.jQueryTab.defaults = {
			...
			cookie: {																							// cookie
				...
				expires: 2,																						// when it expires in days or standard UTC time
				path: '/',																						// path on which cookie is accessible
				domain:'',																						// domain of the cookie
				secure: true																					// enable secure cookie - requires https connection to transfer
			},
